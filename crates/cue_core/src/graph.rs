...
impl DependencyGraph {
        ...
        for doc in docs {
            ...
            for link in &doc.links {
                ...
                let target_path = if link.starts_with("./") || link.starts_with("../") {
                    ...
                    file_map.get(&link.to_lowercase()).cloned()
    ...
    pub fn sort_topological(&self) -> Result<Vec<PathBuf>> {
        match toposort(&self.graph, None) {
            Ok(sorted_nodes) => {
                ...
                let mut result: Vec<PathBuf> = sorted_nodes.into_iter()
    ...
    pub fn orphans(&self) -> Vec<PathBuf> {
        self.graph
            .node_indices()
            .filter(|&node| self.graph.neighbors_directed(node, petgraph::Direction::Incoming).count() == 0)
            .map(|node| self.graph[node].clone())
            .collect()
    }
