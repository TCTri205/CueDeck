...
import { TaskTreeItem } from './providers/TaskTreeItem';
...
export function registerCommands(
    ...
    taskTreeProvider?: any  // Will be passed from extension.ts
) {
    // Search command
    ...
    const changeAssigneeCommand = vscode.commands.registerCommand(
        'cuedeck.changeTaskAssignee',
        async (item: TaskTreeItem) => {
            if (!item.task) {
                return;
            }
    ...
    const createFromTemplateCommand = vscode.commands.registerCommand(
        'cuedeck.createTaskFromTemplate',
        async () => {
    ...
    const filterTasksCommand = vscode.commands.registerCommand(
        'cuedeck.filterTasks',
        async () => {
            if (!taskTreeProvider) return;
            ...
            switch (filterType.value) {
                case 'priority':
                    ...
                    if (priority) {
                        taskTreeProvider.setFilters({ priority: priority.value as any });
                ...
                case 'tags':
                    ...
                    if (selectedTags) {
                        ...
                        vscode.window.showInformationMessage(
                            `Filtered by tags: ${selectedTags.map(t => t.label).join(', ')}`
                        );
                    }
                    break;
            }
    ...
    // Sort tasks command
    const sortTasksCommand = vscode.commands.registerCommand(
        'cuedeck.sortTasks',
        async () => {
            if (!taskTreeProvider) return;
            ...
            if (sortBy) {
                taskTreeProvider.setSortOptions(sortBy.value as any);
...
async function getUniqueAssignees(client: CuedeckClient): Promise<string[]> {
...
async function getUniqueTags(client: CuedeckClient): Promise<string[]> {
    const result = await client.listTasks('all');
    if (!result.success || !result.data) return [];
 ...
 */
async function searchDocuments(client: CuedeckClient, context?: vscode.ExtensionContext) {
    // Get recent searches from workspace state
    const recentSearches = context?.workspaceState.get<string[]>('recentSearches', []) || [];

    // Build search query options 
    const queryItems: vscode.QuickPickItem[] = [];
    ...
    const mode = selectedMode.label.toLowerCase() as 'hybrid' | 'semantic' | 'keyword';
    ...
    await vscode.window.withProgress(
        ...
        async () => {
            ...
            if (selected && (selected as any).path) {
                // Open selected file
                const uri = vscode.Uri.file((selected as any).path);
                const document = await vscode.workspace.openTextDocument(uri);
                await vscode.window.showTextDocument(document);
            }
        }
 ...
 */
async function getFilePreview(filePath: string, query?: string): Promise<string | null> {
    try {
